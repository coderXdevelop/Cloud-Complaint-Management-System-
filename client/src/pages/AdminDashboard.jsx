...
import { Search, Filter, FileText, Clock, CheckCircle2, Loader2, Image as ImageIcon, StickyNote, RefreshCw, Star, Trophy, ClipboardList, TrendingUp } from 'lucide-react';
...
const DonutChart = ({ pending = 0, processing = 0, resolved = 0 }) => {
  const total = pending + processing + resolved;
  if (total === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-48 text-gray-450 dark:text-gray-500 text-sm italic">
  ...
  const pendingStroke = (pendingPct / 100) * circ;
  const processingStroke = (processingPct / 100) * circ;
  const resolvedStroke = (resolvedPct / 100) * circ;

  return (
    <div className="flex flex-col sm:flex-row items-center justify-around gap-6 py-4 w-full">
        ...
        <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90">
          ...
          {pending > 0 && (
            <circle
              ...
              strokeWidth="8"
              fill="transparent"
              strokeDasharray={`${pendingStroke} ${circ}`}
          ...
          {processing > 0 && (
            <circle
              ...
              strokeWidth="8"
              fill="transparent"
              strokeDasharray={`${processingStroke} ${circ}`}
              strokeDashoffset={circ - pendingStroke}
            />
          )}
          {resolved > 0 && (
            <circle
              ...
              strokeWidth="8"
              fill="transparent"
              strokeDasharray={`${resolvedStroke} ${circ}`}
              strokeDashoffset={circ - pendingStroke - processingStroke}
            />
          )}
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-bold text-gray-900 dark:text-white">{total}</span>
          <span className="text-[10px] text-gray-400 uppercase font-medium">Tickets</span>
        </div>
      </div>

      <div className="space-y-2.5 w-full sm:w-auto min-w-[140px]">
        <div className="flex items-center justify-between gap-4 text-xs font-semibold text-gray-500 dark:text-gray-400">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-500 flex-shrink-0" />
            <span>Pending</span>
          </div>
          <span className="text-gray-900 dark:text-white">{pending} ({pendingPct.toFixed(0)}%)</span>
        </div>
        <div className="flex items-center justify-between gap-4 text-xs font-semibold text-gray-500 dark:text-gray-400">
          <div className="flex items-center gap-2">
          ...
          <span className="text-gray-900 dark:text-white">{processing} ({processingPct.toFixed(0)}%)</span>
        </div>
        <div className="flex items-center justify-between gap-4 text-xs font-semibold text-gray-500 dark:text-gray-400">
          <div className="flex items-center gap-2">
          ...
          <span className="text-gray-900 dark:text-white">{resolved} ({resolvedPct.toFixed(0)}%)</span>
        </div>
      </div>
    </div>
  );
...
// Custom HTML/CSS Bar Chart for status volume comparison
const BarChart = ({ pending = 0, processing = 0, resolved = 0 }) => {
  const total = pending + processing + resolved;
  if (total === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-48 text-gray-450 dark:text-gray-500 text-sm italic">
  ...
  const maxVal = Math.max(pending, processing, resolved, 1);
  const data = [
    { label: 'Pending', count: pending, bg: 'bg-amber-500' },
    { label: 'In Progress', count: processing, bg: 'bg-blue-500' },
    { label: 'Resolved', count: resolved, bg: 'bg-emerald-500' }
  ];

  return (
    <div className="flex items-end justify-center gap-8 h-44 w-full px-4">
      {data.map((item, idx) => {
        const heightPercent = (item.count / maxVal) * 100;
        return (
          <div key={idx} className="flex flex-col items-center flex-1 max-w-[60px] h-full justify-end group">
            <span className="text-xs font-bold text-gray-900 dark:text-white mb-2 transform group-hover:scale-110 transition-transform">
              {item.count}
            </span>
            <div className="w-6 sm:w-8 bg-gray-100 dark:bg-gray-800 rounded-t-lg h-28 flex items-end overflow-hidden relative shadow-inner">
              <div
                style={{ height: `${heightPercent}%` }}
                className={`w-full ${item.bg} rounded-t-md transition-all duration-700 ease-out origin-bottom`}
              />
            </div>
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-2 truncate w-full text-center">
              {item.label}
            </span>
          </div>
        );
...
const AdminDashboard = () => {
  ...
  const isDashboardView = location.pathname === '/admin/dashboard';
  ...
  return (
    <Layout title={isDashboardView ? "Faculty Analytics" : "Manage Complaints"}>
      <div className="space-y-6 animate-slide-in">
        {/* Page Header */}
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
            {isDashboardView ? "Faculty Analytics" : "Manage Complaints"}
          </h2>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            {isDashboardView 
              ? "Real-time complaint statistics and overview metrics" 
              : "Review and resolve student reported complaints"}
          </p>
        </div>
        ...
        {isDashboardView ? (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard title="Total Complaints" value={stats?.total || 0} icon={ClipboardList} color="indigo" />
              <StatCard title="Pending Complaints" value={stats?.pending || 0} icon={Clock} color="amber" />
              <StatCard title="In Progress" value={stats?.processing || 0} icon={RefreshCw} color="blue" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="card flex flex-col h-full">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-base font-semibold text-gray-900 dark:text-white">Complaint Status Ratio</h3>
                </div>
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">Distribution breakdown of campus tickets</p>
                <div className="flex-1 flex items-center justify-center">
                  <DonutChart pending={stats?.pending || 0} processing={stats?.processing || 0} resolved={stats?.resolved || 0} />
                </div>
              </div>
              ...
              <div className="card flex flex-col h-full">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-base font-semibold text-gray-900 dark:text-white">Detailed Status Count</h3>
                </div>
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">Comparison of absolute complaints volume</p>
                <div className="flex-1 flex items-center justify-center">
                  <BarChart pending={stats?.pending || 0} processing={stats?.processing || 0} resolved={stats?.resolved || 0} />
                </div>
              </div>
            </div>
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {c.image && (
                                <button onClick={() => setImgModal(c.image.startsWith('http') ? c.image : `/uploads/${c.image}`)}
                                  className="p-1.5 rounded-lg text-gray-400 hover:text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-950/20 transition-all" title="View image">
